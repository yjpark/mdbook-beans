...
/// Render a full bean page with metadata and body.
pub fn render_bean_page(bean: &Bean, all_beans: &[Bean]) -> String {
    ...
    if let Some(parent_id) = &bean.frontmatter.parent {
        page.push_str(&format!("| **Parent** | [{parent_id}]({parent_id}.md) |\n"));
    }
